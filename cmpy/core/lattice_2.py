...
class Lattice:
    ...
    def show(self, show=True, size=10., color=True, margins=1., show_hop=True, lw=1.):
        ...
        positions = np.asarray([self.get_position(i) for i in range(self.n)])
    ...
    def distance(self, idx0, idx1):
        ...
        distance: float
        """
        r1 = self.position(*idx0)
        r2 = self.position(*idx1)
    ...
    def calculate_distances(self, n=1):
        ...
        r_vecs = [self.position(*idx) for idx in iter_indices(n_vecs, self.n_base)]
    ...
    # def _build_positions(self, n_vecs):
    #     indices = list()
    #     for n in n_vecs:
    #         for alpha in range(self.n_base):
    #             indices.append([*n, alpha])
    ...
    def _check_nvec(self, n, width, height):
    ...
    def build_rect(self, width, height):
        ...
        final_nvecs = list()
        for n in n_vecs:
            if self._check_nvec(n, width, height):
...
def main():
    latt = Lattice(np.eye(2) * 1)
    latt.add_atom("A", neighbour_dist=3)
    print(latt.distances)
    for i in range(5):
        print(latt.position([i, 0]))
