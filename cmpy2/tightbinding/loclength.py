...
def read_file_localization(path):
    data = LT_Data(path)
    info = data.info()
    keys = data.keylist
    n = len(keys)
    ...
    for i in range(n):
        ...
        try:
            ...
            ll, ll_err = loc_length(l, t)
            arr[i] = w, ll, ll_err
        except Exception as e:
            print(f"Error (Key: {k})" + str(e))
    idx = np.argsort(arr[:, 0])
    arr = arr[idx]
    return info, arr.T


def get_localization_transdata(root):
    folder = Folder(root)
    paths = list()
    for path in folder.walk_files():
        dirname, fn = os.path.split(path)
        if fn.startswith("disorder"):
            paths.append(path)
    return paths


def save_localization_data(path):
    for subfolder in Folder(path).subfolders():
        root = subfolder.path
        print(f"Reading folder {subfolder.name}")
        for path in get_localization_transdata(root):

            info, arr = read_file_localization(path)

            print("   Saving Localization:", info)
            h = int(info["h"])
            e = info["e"]
            try:
                soc = info["soc"]
                fn = f"loclen-e={e}-soc={soc}.npz"
            except KeyError:
                fn = f"loclen-e={e}.npz"

            data = LoclenData(root, fn)
...
def get_loclen_files(path, basis=None, soc=None, e=None):
    folder = Folder(path)
    # Get basis folders
    if basis is None:
        ...
        for subfolder in folder.subfolders():
            if basis in subfolder.name:
                folders.append(subfolder)

    txts = list()
    if soc is not None:
        txts.append(f"soc={soc}")
    if e is not None:
        txts.append(f"e={e}")

    paths = list()
    for folder in folders:
        paths += folder.find("loclen", *txts)
    return paths
